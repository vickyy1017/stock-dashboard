...
type TabType = "fundamental" | "technical" | "trade" | "compare" | "ranking" | "board";
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-bold text-zinc-100">综合评分分析</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {allPatterns.map((p, i) => <PatternTag key={i} pattern={p} />)}
      </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        <div className="w-1 h-4 rounded-sm bg-blue-500" />
        <span className="text-sm font-bold text-zinc-100">13只股票横向对比</span>
        <span className="text-xs text-zinc-500 ml-1">（点击表头排序，点击行查看详情）</span>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr>
              ...
              <th className={thClass}>技术形态</th>
              <th className={thClass}>评级</th>
              ...
              return (
                <tr key={r.ticker} onClick={() => onSelectTicker(r.ticker)}
                  ...
                  <td className="px-3 py-2.5">
                    <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background: `${r.color}22`, color: r.color, border: `1px solid ${r.color}44` }}>{r.ticker}</span>
                  </td>
                  <td className="px-3 py-2.5 text-zinc-400">{r.name}</td>
                  <td className="px-3 py-2.5 text-zinc-300">{r.market_cap}</td>
                  <td className="px-3 py-2.5">
                    <span className={r.pe < 35 ? "text-green-400" : r.pe < 80 ? "text-yellow-400" : "text-red-400"}>{r.pe}x</span>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 rounded-full" style={{ width: `${(r.net_margin / maxMargin) * 60}px`, background: r.net_margin > 20 ? "#4ade80" : r.net_margin > 5 ? "#fbbf24" : "#f87171", minWidth: 2 }} />
                      <span className={r.net_margin > 20 ? "text-green-400" : r.net_margin > 5 ? "text-yellow-400" : "text-red-400"}>{r.net_margin}%</span>
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 rounded-full bg-blue-500" style={{ width: `${(r.revenue_growth / maxGrowth) * 60}px`, minWidth: 2 }} />
                      <span className="text-blue-400">{r.revenue_growth}%</span>
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 rounded-full" style={{ width: `${(r.score / 25) * 60}px`, background: sc }} />
                      <span className="font-bold" style={{ color: sc }}>{r.score}</span>
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex flex-wrap gap-1">
                      {allPat.map((p, i) => <PatternTag key={i} pattern={p.length > 12 ? p.slice(0, 12) + "…" : p} />)}
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <span className="px-2 py-0.5 rounded text-xs font-semibold" style={{ background: rs.bg, color: rs.text }}>{r.rating}</span>
                  </td>
...
function RankingPanel({ onSelectTicker }: { onSelectTicker: (t: string) => void }) {
  const ranked = useMemo(() =>
    Object.entries(STOCKS).map(([t, d]) => ({ ticker: t, ...d })).sort((a, b) => b.score - a.score),
    []
  );
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        <div className="w-1 h-4 rounded-sm bg-blue-500" />
        <span className="text-sm font-bold text-zinc-100">综合排名（基本面 + 技术面）</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {ranked.map((d, i) => {
          ...
          return (
            <div key={d.ticker} onClick={() => onSelectTicker(d.ticker)}
              className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 cursor-pointer hover:border-blue-500/50 hover:bg-zinc-800/60 transition-all group">
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {allPat.map((p, j) => <PatternTag key={j} pattern={p.length > 14 ? p.slice(0, 14) + "…" : p} />)}
              </div>
              <a href={tvUrl(d.ticker)} target="_blank" rel="noopener noreferrer"
                onClick={e => e.stopPropagation()}
                className="flex items-center gap-1 text-xs text-zinc-600 hover:text-blue-400 transition-colors">
                <ExternalLink size={11} /> 在 TradingView 查看
              </a>
            </div>
          );
        })}
...
export default function Home() {
  ...
  const handleSelectTicker = (t: string) => {
    setSelectedTicker(t);
    if (activeTab === "board") setActiveTab("fundamental");
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      <aside className="w-64 shrink-0 bg-zinc-900 border-r border-zinc-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {filteredTickers.map(t => {
            ...
            const sc = getScoreColor(d.score);
            const rs = getRatingStyle(d.rating);
            const isActive = selectedTicker === t && activeTab !== "board";
            return (
        ...
        <div className="p-2 border-t border-zinc-800">
          <button
            ...
            className={`w-full px-3 py-2.5 rounded-lg text-xs font-semibold border transition-all ${activeTab === "board" ? "bg-blue-950 border-blue-600 text-blue-300" : "bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-zinc-200 hover:border-zinc-600"}`}
          >
            💬 申请新股票分析
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-zinc-900 border-b border-zinc-800 px-5 py-3 flex items-center justify-between shrink-0">
          {activeTab === "board" ? (
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-bold text-zinc-100">股票申请留言板</div>
                <div className="text-xs text-zinc-500">告诉我们你想分析哪只股票</div>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3">
          ...
          <nav className="flex bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden">
            {tabs.filter(t => t.key !== "board").map(tab => (
        ...
        <main className="flex-1 overflow-y-auto p-5">
          ...
          {activeTab === "fundamental" && <FundamentalPanel ticker={selectedTicker} data={currentData} />}
          {activeTab === "technical"   && <TechnicalPanel ticker={selectedTicker} data={currentData} />}
          {activeTab === "trade"       && (
            TRADE_ANALYSIS[selectedTicker]
              ? <TradePanel data={TRADE_ANALYSIS[selectedTicker]} />
              : <div className="text-center py-16 text-zinc-500">暂无该股票的交易分析数据</div>
          )}
          {activeTab === "compare"     && <ComparePanel onSelectTicker={t => { handleSelectTicker(t); }} />}
          {activeTab === "ranking"     && <RankingPanel onSelectTicker={t => { handleSelectTicker(t); }} />}
        </main>
      </div>
    </div>
  );
